...
<body>
    ...
    <ul class="nav navbar-nav navbar-right">
        <li><a href="<?=FOLDER_PATH . 'Login'?>">Iniciar sesión</a></li>
        <li><a href="<?=FOLDER_PATH . 'Home/formularioRegistro'?>">Registrarme</a></li>
    </ul>
    </nav>
    <?php
        for ($i=0; $i<count($info_plato); $i++){
            echo '<blockquote>';
            echo '<p><h2>'.$info_plato[$i][1].'</h2></p>';
            echo '<footer>'.$info_plato[$i][2].'</footer>';
            echo '</blockquote>';
        ?>
  ...
  <div class="carousel-inner" role="listbox">
      <?php
      $contador = 0;
      while ($row = $info_plato[$i][3]->fetch_assoc())
      {
          if($contador == 0){
              $nameFoto = $row['Foto'];
              echo
